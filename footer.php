<footer>
    <div class="container">
      <div class="row">
      
      
        <div class="col-xs-12 col-sm-3 col-md-3">
          <span class="phone"><u>HYDERABAD :</u><p>&nbsp;</p><i class="fa fa-map-marker"></i> Opp : Sub Registrar Office, Ashok Nagar, Hyderabad-500020. <br>
            <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=1"> Route Map from HYD Bus Stop</a><br>
            <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=2"> Route Map from HYD Railway Statio</a><br>
            <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=3"> Route Map from SEC Bus Stop</a><br>
            <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=4"> Route Map from SEC Railway Statio</a><br><p>&nbsp;</p>

            <i class="fa fa-map-marker"></i> Opp.UmeshChandraStatue, Vodafone-Airtel Building, 3rd Floor, SR Nagar Busstop, SR Nagar, Hyderabad-500038<br>
           <i class="fa fa-phone"></i>  08686233879 | 040 - 65537733 | 09972075540<br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=5"> Route Map from HYD Bus Stop</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=6"> Route Map from HYD Railway Station</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=7"> Route Map from SEC Bus Stop</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=8"> Route Map from SEC Railway Station</a><br><p>&nbsp;</p>
          </span>
        </div>
        <div class="col-xs-12 col-sm-3 col-md-3">
          <span class="phone"><u>HYDERABAD :</u><p>&nbsp;</p>
            <i class="fa fa-map-marker"></i> Beside VRK Silks, Archids Building, Opp.Chutneys Restaurant, KPHB, Hyderabad-500072. <br>
           <i class="fa fa-phone"></i>   09502318332<br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=9"> Route Map from HYD Bus Stop</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=10"> Route Map from HYD Railway Station</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=11"> Route Map from SEC Bus Stop</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=12"> Route Map from SEC Railway Station</a><br><p>&nbsp;</p>
           </span>
        </div><div class="col-xs-12 col-sm-3 col-md-3">
          <span class="phone"><u>VISAKHAPATNAM :</u><p>&nbsp;</p>
            <i class="fa fa-map-marker"></i> Opp : Union Bank, Janardhan Arcade, 3rd Floor, 4th Lane Dwaraka Nagar, Visakhapatnam-530016.<br>
           <i class="fa fa-phone"></i> 09247000899<br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=13">  Route Map from Bus Stop </a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=14">  Route Map from Railway Station </a><br><p>&nbsp;</p>
           </span>
        </div><div class="col-xs-12 col-sm-3 col-md-3">
          <span class="phone"><u>RAJAHMUNDRY :</u><p>&nbsp;</p>
            <i class="fa fa-map-marker"></i> Opp: Panchayat School, Torredu(v), Seetanagaram Road, Rajahmundry-533104.<br>
           <i class="fa fa-phone"></i> 9573416524, 9618883155, 9618885155<br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=15"> Route Map from Bus Stop</a><br>
           <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=16"> Route Map from Railway Station</a><br><p>&nbsp;</p>
           </span>
           <span class="phone"><u>VIJAYAWADA :</u><p>&nbsp;</p>
            <i class="fa fa-map-marker"></i> #40-1-50/1, 2nd Floor, OPP : State Bank of India, Bandar Road, Vijayawada - 520010.<br>
                <i class="fa fa-phone"></i> 09133678666, 09133351165<br>
                <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=17"> Route Map from Bus Stop</a><br>
                <i class="fa fa-location-arrow" style="color: red;"></i><a href="Route-Map.php?id=18">Route Map from Railway Station</a><br><p>&nbsp;</p>
              </span>
        </div>
        <div class="col-md-12">
          <div class="f-sicon">
            <h2>FOLLOW US ON</h2>
                     <ul>
                        <li><a href="https://www.facebook.com/21stCenturyIas"><i class="fa fa-facebook" aria-hidden="true"></i></a></li>
                        <li><a href="https://twitter.com/kpiasacademy"><i class="fa fa-twitter" aria-hidden="true"></i></a></li>
                        <li><a href="#"><i class="fa fa-google-plus" aria-hidden="true"></i></a></li>
                        <li><a href="https://www.linkedin.com/in/krishna-pradeep-872107106/"><i class="fa fa-linkedin" aria-hidden="true"></i></a></li>
                        <li><a href="#"><i class="fa fa-instagram" aria-hidden="true"></i></a></li>
                         <li><a href="https://www.youtube.com/user/kptrainer21/videos" target="_blank"><i class="fa fa-youtube-play" aria-hidden="true"></i></a></li>
                     </ul>
                  </div>
          <div class="copyright">
            &copy;KPIAS. All Right Reserved 2017<small>.Designed and Developed by <a href="http://www.21cssindia.com" style="color:white">21CSS India Pvt. Ltd.</a></small><br> 
            <h6 style="display: none;"><u><a href="#" style="color:white;">SiteMaps </a></u></h6>
          </div>
        </div>
      </div>
    </div>
  </footer>
  